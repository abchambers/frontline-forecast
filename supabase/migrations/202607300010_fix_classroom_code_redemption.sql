-- Qualify the code table reference: return-column names are variables inside
-- PL/pgSQL functions and must not shadow the stored classroom reference.
...
begin
  ...
  where workspace_join_codes.code_hash = normalized_hash
    and workspace_join_codes.classroom_id is not null
  for update;
  ...
  select * into entitlement from public.organization_entitlements where organization_id = classroom.organization_id for update;
  ...
  select id into redemption_id from public.classroom_code_redemptions
  ...
  return query select organization.id, organization.name, classroom.id, classroom.name
  from public.organizations organization where organization.id = classroom.organization_id;
end;
$$;
