-- Use named unique constraints so the enrollment function's returned column
-- names cannot shadow the conflict targets.
...
begin
  ...
  on conflict on constraint organization_memberships_organization_id_user_id_key do update set status = 'active';
  insert into public.classroom_memberships (classroom_id, user_id, role, status)
  values (classroom.id, auth.uid(), 'student', 'active')
  on conflict on constraint classroom_memberships_classroom_id_user_id_key do update set status = 'active';
