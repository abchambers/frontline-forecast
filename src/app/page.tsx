...
type ClassroomAssignment = { id: string; classroom_id: string; title: string; instructions: string | null; target_date: string; target_dates?: string[]; scenario_id?: string | null; scenario?: { title: string; summary: string | null; reference_notes: string | null; reference_links: ScenarioReferenceLink[] } | null; due_at: string | null; status: "draft" | "open" | "closed"; instructor_forecast: InstructorForecastSnapshot | null; instructor_forecast_updated_at: string | null; class_forecast: ClassForecastSnapshot | null; class_forecast_updated_at: string | null; class_forecast_published_at: string | null; created_at: string };
...
type WorkspacePreferences = { defaultLocationId: string; radarMapView: RadarMapView; radarOpacity: number; showNwsAlerts: boolean; showSpcOutlook: boolean; showSevereMarkers: boolean; radarProviderPreference: RadarProviderPreference; defaultForecastDays: 1 | 3 | 7 };
...
function RadarControlsMenu({
  ...
  showOutlookToggle = false, showSpcOutlook = false, onToggleOutlook,
...
function SchoolDesk({ workspace, classrooms, members, codes, entitlement, onOpenClassroom, onCreateClassroom, onRenameClassroom, onAssignInstructor, onCreateCode, onRetireCode, message }: { workspace: WorkspaceContext; classrooms: WorkspaceContext[]; members: OrganizationMember[]; codes: ClassroomJoinCode[]; entitlement: { class_limit: number; class_seat_limit: number; status: string; next_payment_due_at: string | null } | null; onOpenClassroom: (workspace: WorkspaceContext) => void; onCreateClassroom: (name: string, term: string, seatLimit?: number) => void; onRenameClassroom: (classroom: WorkspaceContext, name: string, term: string) => void; onAssignInstructor: (classroomId: string, userId: string) => void; onCreateCode: (classroomId: string, label: string, maxUses: number | null, expiresAt: string | null) => Promise<string | null>; onRetireCode: (id: string) => void; message: string }) {
...
function ClassroomAssignmentStudio({ assignments, submissions, roster, selectedAssignmentId, canManage, canOpenForecast, onCreate, onSelectAssignment, onOpenForecast, message }: { assignments: ClassroomAssignment[]; submissions: ClassroomAssignmentSubmission[]; roster: AcademicRosterMember[]; selectedAssignmentId: string; canManage: boolean; canOpenForecast: boolean; onCreate: (fields: ClassroomAssignmentFields) => void; onSelectAssignment: (assignment: ClassroomAssignment) => void; onOpenForecast: () => void; message: string }) {
  ...
  return <section className="assignment-studio">
    ...
    <section className="assignment-rail"><header><h3>All assignments</h3></header><div>{assignments.map((assignment) => <button type="button" key={assignment.id} className={assignment.id === selectedAssignmentId ? "active" : ""} onClick={() => onSelectAssignment(assignment)}><span><strong>{assignment.title}</strong><small>{assignmentDates(assignment).map(forecastTargetTitle).join(" · ")}</small></span><b>{assignment.status}</b></button>)}{!assignments.length && <p className="empty">No assignments have been created yet.</p>}</div></section>
    {selectedAssignment && <section className="assignment-focus"><header><div><p className="eyebrow">Selected assignment</p><h3>{selectedAssignment.title}</h3><p>{selectedAssignment.instructions || "No additional directions were added."}</p><small>{selectedDates.map(forecastTargetTitle).join(" · ")}{selectedAssignment.due_at ? ` · due ${new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", minute: "2-digit", timeZone: "America/New_York" }).format(new Date(selectedAssignment.due_at))}` : ""}</small></div>{canOpenForecast && <button type="button" onClick={onOpenForecast}>{canManage ? "Build example" : "Start assignment"}</button>}</header>
      {selectedAssignment.scenario && <div className="assignment-linker scenario-context"><div><strong>Historical scenario</strong><small>This date has already happened — it's a real past event, not a hypothetical.</small>{selectedAssignment.scenario.summary && <em>{selectedAssignment.scenario.summary}</em>}{(selectedAssignment.scenario.reference_notes || selectedAssignment.scenario.reference_links.length > 0) && <details className="scenario-reference-details"><summary>Reference data</summary>{selectedAssignment.scenario.reference_notes && <p>{selectedAssignment.scenario.reference_notes}</p>}{selectedAssignment.scenario.reference_links.length > 0 && <ul>{selectedAssignment.scenario.reference_links.map((link) => <li key={link.label}>{link.label}{link.detail ? ` — ${link.detail}` : ""}{link.url && <> · <a href={link.url} target="_blank" rel="noreferrer">Open</a></>}</li>)}</ul>}</details>}</div></div>}
...
export default function Home() {
  ...
  const [schoolEntitlement, setSchoolEntitlement] = useState<{ class_limit: number; class_seat_limit: number; status: string; next_payment_due_at: string | null } | null>(null);
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(workspaceSettingsStorageKey, JSON.stringify({ defaultLocationId, radarMapView, radarOpacity, showNwsAlerts, showSpcOutlook, showSevereMarkers, radarProviderPreference, defaultForecastDays } satisfies WorkspacePreferences));
  }, [defaultForecastDays, defaultLocationId, radarMapView, radarOpacity, radarProviderPreference, settingsReady, showNwsAlerts, showSpcOutlook, showSevereMarkers]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`${supabaseUrl}/rest/v1/organization_entitlements?select=class_limit,class_seat_limit,status,next_payment_due_at&organization_id=eq.${organizationId}`, { headers }),
    ]).then(async ([membersResponse, codesResponse, entitlementResponse]) => {
      ...
      const entitlementRows = entitlementResponse.ok ? await entitlementResponse.json() as { class_limit: number; class_seat_limit: number; status: string; next_payment_due_at: string | null }[] : [];
  ...
  return (
    ...
    <main className={`app desk-${activeWorkspace?.kind ?? "public"}`}>
      ...
      <section className="home-intro"><div className="home-intro-copy"><div className="clock-line" aria-live="polite"><span>Local time <strong>{localTimeLabel}</strong></span><span>UTC <strong>{utcTimeLabel}</strong></span></div></div></section>
      ...
      {activeSection === "verify" && session && verifyTab === "records" && <section className="workspace-card">
        ...
        <section className="record-calendar" aria-label="Forecast record dates"><div className="record-calendar-heading"><div><strong>{new Intl.DateTimeFormat("en-US", { month: "long", year: "numeric", timeZone: "America/New_York" }).format(new Date(`${recordWindowStart}T12:00:00`))}</strong><small>Forecast target dates</small></div><div><button type="button" aria-label="Previous seven days" onClick={() => setRecordWindowStart(addDays(new Date(`${recordWindowStart}T12:00:00`), -7))}>←</button><button type="button" aria-label="Next seven days" onClick={() => setRecordWindowStart(addDays(new Date(`${recordWindowStart}T12:00:00`), 7))}>→</button></div></div><div className="day-outlook-cards record-calendar-days">{recordWindowDates.map((targetDate) => { const archive = archiveForDate(targetDate); const verification = archive ? automaticVerifications[archive.id] : null; return <button type="button" key={targetDate} className={`${archive?.id === selectedArchiveId ? "active " : ""}${archive ? "has-record" : ""}`} onClick={() => archive && setSelectedArchiveId(archive.id)} disabled={!archive}><span className="dow">{new Intl.DateTimeFormat("en-US", { weekday: "short", timeZone: "America/New_York" }).format(new Date(`${targetDate}T12:00:00`))}</span>{archive ? <img src={`/weather-icons/${weatherIconStyle}/${periodIconCondition(archive.day)}.svg`} alt="" /> : <strong>{new Intl.DateTimeFormat("en-US", { day: "numeric", timeZone: "America/New_York" }).format(new Date(`${targetDate}T12:00:00`))}</strong>}{archive ? <><em>{displayForecastTemperature(archive.day.high)} / {displayForecastTemperature(archive.night.low)}</em><small>PoP {displayForecastChance(archive.day.rainChance)}/{displayForecastChance(archive.night.rainChance)}</small><small>{verification?.dayScore !== null && verification?.dayScore !== undefined ? `Score ${verification.dayScore}%` : "Unscored"}</small></> : <small>No forecast</small>}</button>; })}</div></section>
