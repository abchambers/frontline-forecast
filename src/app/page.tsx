...
type RadarTimelineFrame = { time: number; tileUrl: string; source?: "provider" | "nexrad" | "mosaic"; inHouseTime?: string };
...
type SavedForecast = {
  ...
  day: { high: string; conditions: string; rainChance: string; timing: string; hazards: string; wind?: string; dewpoint?: string; reasoning?: string; references?: ReferenceItem[]; iconCondition?: string };
  night: { low: string; conditions: string; rainChance: string; timing: string; hazards: string; wind?: string; dewpoint?: string; reasoning?: string; references?: ReferenceItem[]; iconCondition?: string };
...
type PeriodDraft = { highLow: string; conditions: string; rainChance: string; timing: string; wind: string; dewpoint: string; confidence: string; hazards: string; reasoning: string; references: ReferenceItem[]; iconCondition: string };
...
type AccentPreference = "default" | "deep-navy" | "storm-red";
type CardStyle = "flat" | "raised";
...
type ClassForecastDay = { date: string; submitted_count: number; day: { high_f: number | null; pop: number | null; conditions: string[]; wind: string[]; dewpoint_f: number | null }; night: { low_f: number | null; pop: number | null; conditions: string[]; wind: string[]; dewpoint_f: number | null } };
type ClassForecastSnapshot = { generated_at: string; target_date: string; submitted_count: number; total_students: number; day: { high_f: number | null; pop: number | null; conditions: string[]; wind: string[]; dewpoint_f: number | null }; night: { low_f: number | null; pop: number | null; conditions: string[]; wind: string[]; dewpoint_f: number | null }; days?: ClassForecastDay[] };
...
type WorkspacePreferences = { defaultLocationId: string; radarMapView: RadarMapView; radarOpacity: number; showNwsAlerts: boolean; showSpcOutlook: boolean; outlookDay?: 1 | 2; showSevereMarkers: boolean; radarProviderPreference: RadarProviderPreference; defaultForecastDays: 1 | 3 | 7; accentPreference?: AccentPreference; cardStyle?: CardStyle };
...
const radarSourceLabels: Record<"nexrad" | "mosaic" | "provider", string> = {
  nexrad: "NEXRAD Level II (in-house)",
  mosaic: "NEXRAD Level II (local mosaic)",
...
const defaultHomepageContent: HomepageContent = { title: "Forecast with evidence.", description: "Use live observations, radar, guidance, and verification to make better weather decisions.", primaryAction: "View local weather", secondaryAction: "Sign in to forecast", outlookTitle: "NWS Daily Outlook", outlookCaption: "Local forecast guidance", radarTitle: "Radar", radarCaption: "Live composite reflectivity", referenceTitle: "Forecast data", referenceCaption: "Reference data for your next forecast.", showOutlook: true, showRadar: true, showReferences: true };
...
function emptyPeriod(period: "day" | "night"): PeriodDraft {
  return { highLow: "", conditions: "", rainChance: "", timing: "", wind: "", dewpoint: "", confidence: "moderate", hazards: "", reasoning: "", references: [], iconCondition: "" };
}
...
// omits that readout.
function RadarLegendStrip({ view, inline = false, elevationDeg = null, observedAtLabel = null, satelliteLabel = "GOES-East" }: { view: RadarMapView; inline?: boolean; elevationDeg?: number | null; observedAtLabel?: string | null; satelliteLabel?: string }) {
  ...
  if (view === "satellite") return <div className={className}><span className="radar-source-note">{satelliteLabel} GeoColor · visible daytime / infrared nighttime · refreshes about every 10 min</span></div>;
...
function RadarControlsMenu({
  ...
  hasForecasterToolsAccess, onLogin,
  caption,
}: {
  ...
  hasForecasterToolsAccess: boolean;
  onLogin: () => void;
  caption: string;
}) {
  const isForecast = radarMapView === "future_reflectivity";
  const activeProductLabel = radarMapView === "satellite" ? "Satellite" : isForecast ? "Forecast" : reflectivityLabel;
  // Andrew, live: with the menu closing itself only on specific control changes, tapping anywhere
  // else on the page (the map, a nav link) left it hanging open — RadarScope's own control panel
  // dismisses on any outside tap. `<details>` has no such behavior built in, so this listens for a
  // click that lands outside the element and closes it manually.
  ...
  return <details ref={detailsRef} className="radar-tools"><summary aria-label={`Radar controls — currently showing ${activeProductLabel}`}><span className="radar-tools-current">{activeProductLabel}</span> ☰</summary><div>
    <div className="radar-product-picker"><span>Data layer</span>{isForecast
      ...
      // Velocity dropped from this picker (not deleted — see radar-map.tsx's moment=velocity path):
      // it only ever exists via in-house NEXRAD, with no IEM/provider equivalent to fall back to
      // and no scrubbable timeline behind it either, unlike Composite/Satellite. Andrew, live: showing
      // it as an equal option next to two fully-working ones was advertising a product that, from a
...
// Andrew, 2026-08-28: Forecast/Fronts/Upper air stay visible to everyone regardless of sign-in —
// per his explicit choice, a signed-out visitor should see these exist (not have them silently
// disappear) and get an upsell card when they pick one, not a hidden button. The actual access
// check happens where each tab's content renders (see hasForecasterToolsAccess in the parent).
function RadarProductStrip({ radarMapView, onSelectView, radarWorkspaceTab, onSelectWorkspaceTab }: { radarMapView: RadarMapView; onSelectView: (view: RadarMapView) => void; radarWorkspaceTab: "radar" | "fronts" | "upper-air"; onSelectWorkspaceTab: (tab: "radar" | "fronts" | "upper-air") => void }) {
  ...
  return <div className="radar-product-strip"><div className="radar-field-picker radar-mode-toggle">
    ...
    <button type="button" className={isForecast ? "active forecast" : "forecast"} onClick={() => { onSelectWorkspaceTab("radar"); onSelectView("future_reflectivity"); }}>Forecast</button>
...
// Andrew, 2026-08-28: replaces the old "Personal+" paywall messaging — forecaster tools and model
// data are free with any account for now, not gated behind payment (see hasForecasterToolsAccess).
function SignInToolsUpsell({ label, onLogin }: { label: string; onLogin: () => void }) {
  return <section className="model-access-upsell"><p className="eyebrow">Free with an account</p><h3>{label} requires signing in</h3><p>Forecaster tools and model data are free with any account while pricing is still being finalized — the NWS forecast, basic radar, and alerts stay open to everyone, no account needed.</p><button type="button" onClick={onLogin}>Log in</button></section>;
}
...
function archiveRecordsFromRun(run: CloudRunRow): SavedForecast[] {
  ...
  return [...byDate.entries()].map(([targetDate, periods]) => {
    ...
    return {
      ...
      day: { high: dayData.highLow, conditions: dayData.conditions, rainChance: dayData.rainChance, timing: dayData.timing, hazards: dayData.hazards, wind: dayData.wind, dewpoint: dayData.dewpoint, reasoning: dayData.reasoning, references: savedReferences(dayData.references), iconCondition: dayData.iconCondition },
      night: { low: nightData.highLow, conditions: nightData.conditions, rainChance: nightData.rainChance, timing: nightData.timing, hazards: nightData.hazards, wind: nightData.wind, dewpoint: nightData.dewpoint, reasoning: nightData.reasoning, references: savedReferences(nightData.references), iconCondition: nightData.iconCondition },
...
function buildClassForecastSnapshot(assignment: ClassroomAssignment, submissions: ClassroomAssignmentSubmission[], students: AcademicRosterMember[]): ClassForecastSnapshot {
  ...
  const summaryForDate = (date: string): ClassForecastDay => {
    ...
    return { date, submitted_count: latest.filter((submission) => submission.forecast_periods.some((entry) => entry.valid_date === date)).length, day: { high_f: averageForecastValue(day.map((period) => period.highLow)), pop: averageForecastValue(day.map((period) => period.rainChance)), conditions: commonConditions(day), wind: commonWind(day), dewpoint_f: averageForecastValue(day.map((period) => period.dewpoint)) }, night: { low_f: averageForecastValue(night.map((period) => period.highLow)), pop: averageForecastValue(night.map((period) => period.rainChance)), conditions: commonConditions(night), wind: commonWind(night), dewpoint_f: averageForecastValue(night.map((period) => period.dewpoint)) } };
  };
...
function buildLiveClassForecastSnapshot(archives: SavedForecast[], activeDates: string[], students: AcademicRosterMember[]): ClassForecastSnapshot {
  ...
  const summaryForDate = (date: string): ClassForecastDay => {
    ...
    return { date, submitted_count: onDate.length, day: { high_f: averageForecastValue(onDate.map((record) => record.day.high)), pop: averageForecastValue(onDate.map((record) => record.day.rainChance)), conditions: commonConditions(onDate, "day"), wind: commonWind(onDate, "day"), dewpoint_f: averageForecastValue(onDate.map((record) => record.day.dewpoint ?? "")) }, night: { low_f: averageForecastValue(onDate.map((record) => record.night.low)), pop: averageForecastValue(onDate.map((record) => record.night.rainChance)), conditions: commonConditions(onDate, "night"), wind: commonWind(onDate, "night"), dewpoint_f: averageForecastValue(onDate.map((record) => record.night.dewpoint ?? "")) } };
  };
  const days = [...activeDates].sort().map(summaryForDate);
  const primary = days[0];
  return { generated_at: new Date().toISOString(), target_date: primary?.date ?? "", submitted_count: latest.length, total_students: studentIds.size, day: primary?.day ?? { high_f: null, pop: null, conditions: [], wind: [], dewpoint_f: null }, night: primary?.night ?? { low_f: null, pop: null, conditions: [], wind: [], dewpoint_f: null }, days };
}
...
export default function Home() {
  ...
  // Deliberately always "dashboard" here, even though the real answer is knowable synchronously
  // on the client -- branching this initializer on typeof window (as it used to) makes SSR and the
  // client's first paint disagree, which is a real hydration mismatch (React discards and
  // regenerates the whole <main> tree on load). Resolving the URL params in the effect below
  // instead means server and client agree on first paint, then flip immediately after mount.
  const [activeSection, setActiveSection] = useState<WorkspaceSection>("dashboard");
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get("view") === "about") setActiveSection("about");
    else if (params.get("class")) setActiveSection("classroom");
  }, []);
  ...
  // Real request, 2026-09-08: GA should see GOES-East, LA should see GOES-West -- was hardcoded to
  // GOES-East everywhere regardless of location. Computed client-side too (not just read from the
  // API response) so the very first paint, before that fetch resolves, already guesses the right
  // satellite from the location's own longitude rather than always defaulting to East.
  const [satelliteInfo, setSatelliteInfo] = useState<{ id: "GOES19" | "GOES18"; label: "GOES-East" | "GOES-West" }>({ id: "GOES19", label: "GOES-East" });
  ...
  const [radarSource, setRadarSource] = useState<"nexrad" | "mosaic" | "provider" | null>(null);
  ...
  const [accentPreference, setAccentPreference] = useState<AccentPreference>("default");
  const [cardStyle, setCardStyle] = useState<CardStyle>("flat");
  ...
  // Andrew's call 2026-08-31: the local mosaic (multi-station composite) is the default radar view
  // for everyone, including anonymous visitors (who have no account to persist a preference to, so
  // they always see this default). Signed-in users can switch back to the single-station view in
  // Control Center; that choice follows their account, same pattern as weatherIconStyle above.
  const [radarViewPreference, setRadarViewPreference] = useState<"mosaic" | "station">("mosaic");
  ...
  const [controlTab, setControlTab] = useState<"profile" | "plan" | "preferences" | "security" | "data">("profile");
  ...
  const [workspaceNotice, setWorkspaceNotice] = useState<{ title?: string; message: string; targetDate?: string } | null>(null);
  ...
  // Andrew, 2026-08-28: soft launch — forecaster tools (model data, fronts, upper-air, and the
  // extra radar overlays) require a free account, not payment, while pricing is still being
  // finalized. A signed-out visitor should only ever see the NWS forecast, the basic radar view,
  // and alerts. `hasModelDataAccess` (paid tier/school/admin) in src/lib/access.ts is untouched and
  // still drives the real "Your plan" account page — when real billing enforcement resumes here,
  // swap this back to `hasModelDataAccess({ personalTier, isPlatformAdmin: hasControlAccess,
  // hasSchoolMembership })`.
  const hasForecasterToolsAccess = Boolean(session);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (settings.accentPreference === "deep-navy" || settings.accentPreference === "storm-red") setAccentPreference(settings.accentPreference);
      if (settings.cardStyle === "raised") setCardStyle(settings.cardStyle);
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(workspaceSettingsStorageKey, JSON.stringify({ defaultLocationId, radarMapView, radarOpacity, showNwsAlerts, showSpcOutlook, outlookDay, showSevereMarkers, radarProviderPreference, defaultForecastDays, accentPreference, cardStyle } satisfies WorkspacePreferences));
  }, [accentPreference, cardStyle, defaultForecastDays, defaultLocationId, radarMapView, radarOpacity, radarProviderPreference, settingsReady, showNwsAlerts, showSpcOutlook, outlookDay, showSevereMarkers]);

  useEffect(() => {
    if (accentPreference === "default") delete document.documentElement.dataset.accent;
    else document.documentElement.dataset.accent = accentPreference;
  }, [accentPreference]);

  useEffect(() => {
    if (cardStyle === "flat") delete document.documentElement.dataset.cards;
    else document.documentElement.dataset.cards = cardStyle;
  }, [cardStyle]);
  ...
  useEffect(() => {
    ...
    let isActive = true;
    const loadRadarTimeline = () => fetch(`/api/radar/frames?station=${selectedLocation.radarSite}${radarViewPreference === "mosaic" ? "&mosaic=1" : ""}`, { cache: "no-store" })
      .then(async (response) => {
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || "Unable to load radar frames");
        if (isActive) {
          const frames = data.frames as RadarTimelineFrame[];
          const inHouseCount = frames.filter((frame) => frame.source === "nexrad" || frame.source === "mosaic").length;
          const inHouseLabel = frames.some((frame) => frame.source === "mosaic") ? "in-house local mosaic" : "in-house NEXRAD";
          setRadarFrames(frames);
          setRadarFrameIndex(Math.max(0, frames.length - 1));
          setRadarTimelineStatus(inHouseCount ? `${frames.length} frames · ${inHouseCount} ${inHouseLabel}, rest via IEM` : `${frames.length} frames · NWS/NEXRAD via IEM`);
        }
      })
  ...
  }, [activeSection, selectedLocation, radarViewPreference]);
  ...
  // Client-side guess from the location's own longitude, updated the instant location changes —
  // independent of (and ahead of) the frames fetch below, so the very first paint after switching
  // to Los Angeles already shows GOES-West rather than lagging one round-trip behind on GOES-East.
  useEffect(() => {
    setSatelliteInfo(satelliteForLongitude(selectedLocation.longitude));
  }, [selectedLocation.longitude]);

  useEffect(() => {
    if (radarMapView !== "satellite" && homeRadarMapView !== "satellite") return;
    let isActive = true;
    const loadSatelliteFrames = () => fetch(`/api/satellite/frames?channel=${satelliteChannel}&lon=${selectedLocation.longitude}`, { cache: "no-store" })
      .then(async (response) => {
        const data = await response.json();
        ...
        if (isActive) {
          ...
          // Authoritative — overrides the longitude-based client guess above with whatever the
          // server actually fetched, in case the two ever disagree.
          if (data.satellite && data.satelliteLabel) setSatelliteInfo({ id: data.satellite, label: data.satelliteLabel });
        }
      })
    ...
    return () => { isActive = false; window.clearInterval(refreshId); };
  }, [radarMapView, homeRadarMapView, satelliteChannel, selectedLocation.longitude]);
    ...
    fetch(`${supabaseUrl}/auth/v1/user`, { headers: { apikey: supabaseKey, Authorization: `Bearer ${accessToken}` } })
      ...
      .then((user) => {
        ...
        if (isRecovery || isInvite) {
          ...
          setPasswordMessage(isInvite ? "" : "Reset link verified. Choose a new password below.");
          if (isInvite) {
  ...
  useEffect(() => {
    if (!session || !supabaseUrl || !supabaseKey) return;
    fetch(`${supabaseUrl}/rest/v1/profiles?select=role,weather_icon_style,personal_tier,display_name,radar_view_preference&id=eq.${session.user.id}`, { headers: { apikey: supabaseKey, Authorization: `Bearer ${session.access_token}` } })
      .then((response) => response.ok ? response.json() : [])
      .then((rows: { role: string; weather_icon_style?: WeatherIconStyle | null; personal_tier?: PersonalTier | null; display_name?: string | null; radar_view_preference?: "mosaic" | "station" | null }[]) => {
        ...
        setRadarViewPreference(rows[0]?.radar_view_preference === "station" ? "station" : "mosaic");
        setProfileDraftName(rows[0]?.display_name ?? "");
        setProfileDraftEmail(session.user.email ?? "");
      });
  ...
  useEffect(() => {
    if (activeSection !== "dashboard" || dataPanel !== "models" || !hasForecasterToolsAccess) return;
  ...
  }, [activeSection, dataPanel, openMeteoModel, locationQuery, hasForecasterToolsAccess]);

  useEffect(() => {
    if (activeSection !== "dashboard" || dataPanel !== "models" || openMeteoView !== "compare" || !hasForecasterToolsAccess) return;
    ...
    return () => { active = false; };
  }, [activeSection, dataPanel, openMeteoView, comparisonLeftModel, comparisonRightModel, locationQuery, hasForecasterToolsAccess]);

  useEffect(() => {
    if (activeSection !== "dashboard" || dataPanel !== "ensembles" || !hasForecasterToolsAccess) return;
    let active = true;
    ...
    return () => { active = false; };
  }, [activeSection, dataPanel, locationQuery, hasForecasterToolsAccess]);

  useEffect(() => {
    if (activeSection !== "dashboard" || dataPanel !== "model-sounding" || !hasForecasterToolsAccess) return;
    let active = true;
    ...
    return () => { active = false; };
  }, [activeSection, dataPanel, soundingModel, soundingRunOffset, locationQuery, hasForecasterToolsAccess]);
  ...
  function formatPeriodField(period: "day" | "night", field: "highLow" | "rainChance" | "timing" | "dewpoint") {
    const value = selectedDay[period][field].trim();
    if (!value) return;
    if (field === "highLow" || field === "dewpoint") {
  ...
  async function submitForecastDays(daysToSubmit: ForecastDayDraft[]) {
    ...
    // Real incident (2026-09-08): submitting silently reset the workspace to a fresh,
    // blank draft for the same target date with no confirmation strong enough to stop
    // a second click -- that created a real duplicate "V1" archive with every field
    // empty, which (being the more recent submission) then hid the real forecast on
    // every calendar/summary view that shows "the latest" record for a date. Refusing
    // to submit a day with nothing entered closes off the actual trigger.
    const meaningfulDays = daysToSubmit.filter((day) => day.day.highLow.trim() || day.day.conditions.trim() || day.night.highLow.trim() || day.night.conditions.trim());
    if (!meaningfulDays.length) {
      setSaveMessage(daysToSubmit.length > 1 ? "Nothing to submit — none of the selected days have a high/low or conditions entered yet." : "Nothing to submit — add at least a high/low or conditions before posting this day.");
      return;
    }
    daysToSubmit = meaningfulDays;
    if (!session || !supabaseUrl || !supabaseKey) {
    ...
    try {
      ...
      // A real toast, not just the inline saveMessage text: submitting resets the day
      // card back to a blank draft for the same date right after this, and without a
      // confirmation impossible to miss, that reset reads as "nothing happened" --
      // which is exactly what led to a real accidental duplicate submission.
      setWorkspaceNotice({ title: "Forecast submitted", message: `${detail}.`, targetDate: daysToSubmit.length === 1 ? daysToSubmit[0].date : undefined });
  ...
  function reviseArchive(archive: SavedForecast) {
    ...
    setForecastRun({ id: crypto.randomUUID(), initialHorizonDays: 1, days: [{ date: targetDate, ready: false, day: { ...emptyPeriod("day"), highLow: archive.day.high, conditions: archive.day.conditions, rainChance: archive.day.rainChance, timing: archive.day.timing, hazards: archive.day.hazards, wind: archive.day.wind ?? "", dewpoint: archive.day.dewpoint ?? "", reasoning: archive.day.reasoning ?? "", references: savedReferences(archive.day.references) }, night: { ...emptyPeriod("night"), highLow: archive.night.low, conditions: archive.night.conditions, rainChance: archive.night.rainChance, timing: archive.night.timing, hazards: archive.night.hazards, wind: archive.night.wind ?? "", dewpoint: archive.night.dewpoint ?? "", reasoning: archive.night.reasoning ?? "", references: savedReferences(archive.night.references) } }] });
  ...
  async function saveRadarViewPreference(nextView: "mosaic" | "station") {
    if (!session || !supabaseUrl || !supabaseKey) return;
    const previousView = radarViewPreference;
    setRadarViewPreference(nextView);
    setControlMessage("Saving radar view preference…");
    const response = await fetch(`${supabaseUrl}/rest/v1/profiles?id=eq.${session.user.id}`, {
      method: "PATCH",
      headers: { apikey: supabaseKey, Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json", Prefer: "return=representation" },
      body: JSON.stringify({ radar_view_preference: nextView }),
    });
    if (!response.ok) { setRadarViewPreference(previousView); setControlMessage("Radar view preference could not be saved."); return; }
    setControlMessage(`${nextView === "mosaic" ? "Local mosaic" : "Single station"} radar view saved to your account.`);
  }
  ...
  async function completeAccountSetup() {
    ...
    if (!trimmedEmail) { setPasswordMessage("Enter the email you want to sign in with."); return; }
    setPasswordBusy(true);
    setPasswordMessage("Setting up your account…");
    const emailChanged = trimmedEmail.toLowerCase() !== (session.user.email ?? "").toLowerCase();
    const userUpdate: { password: string; email?: string } = { password: newPassword };
    if (emailChanged) userUpdate.email = trimmedEmail;
    const response = await fetch(`${supabaseUrl}/auth/v1/user`, {
      method: "PUT",
      headers: { apikey: supabaseKey, Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json" },
    ...
    if (!response.ok) {
      const body = await response.json().catch(() => null) as { msg?: string; error_description?: string; message?: string } | null;
      setPasswordBusy(false);
      setPasswordMessage(body?.msg || body?.error_description || body?.message || "Account could not be set up.");
      return;
    }
    ...
    setActiveSection("dashboard");
    setAuthMessage(emailChanged ? "Account created. Check your new email address to confirm the change." : "Account created. Welcome to Frontline Forecast.");
  }

  async function saveProfile() {
    if (!session || !supabaseUrl || !supabaseKey) return;
    const trimmedEmail = profileDraftEmail.trim();
    if (!trimmedEmail) { setMyProfileMessage("Enter the email you sign in with."); return; }
    setMyProfileBusy(true);
    setMyProfileMessage("Saving…");
    const emailChanged = trimmedEmail.toLowerCase() !== (session.user.email ?? "").toLowerCase();
    if (emailChanged) {
      const emailResponse = await fetch(`${supabaseUrl}/auth/v1/user`, {
        method: "PUT",
        headers: { apikey: supabaseKey, Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ email: trimmedEmail }),
      });
      if (!emailResponse.ok) {
        const body = await emailResponse.json().catch(() => null) as { msg?: string; error_description?: string; message?: string } | null;
        setMyProfileBusy(false);
        setMyProfileMessage(body?.msg || body?.error_description || body?.message || "Email could not be updated.");
        return;
      }
    }
    const trimmedName = profileDraftName.trim();
    const nameResponse = await fetch(`${supabaseUrl}/rest/v1/profiles?id=eq.${session.user.id}`, {
      method: "PATCH",
      headers: { apikey: supabaseKey, Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json", Prefer: "return=representation" },
      body: JSON.stringify({ display_name: trimmedName || null }),
    });
    setMyProfileBusy(false);
    if (!nameResponse.ok) { setMyProfileMessage("Name could not be saved."); return; }
    setMyDisplayName(trimmedName || null);
    setMyProfileMessage(emailChanged ? "Saved. Check your new email address to confirm the change." : "Profile saved.");
  }
  ...
  async function createClassroom(organizationId: string, name: string, term: string, seatLimit?: number) {
    if (!session || !supabaseUrl || !supabaseKey) return false;
    ...
    const rows = await response.json().catch(() => []);
    if (!response.ok || !rows[0]?.id) { setAccessMessage(response.status === 400 || response.status === 403 ? "Classroom could not be created. Check the seat count against your school's licensed allocation." : "Classroom could not be created."); return false; }
  ...
  return (
    ...
    <main className={`app desk-${activeWorkspace?.kind ?? "public"}`}>
      <header className="header">
        <h1 className="sr-only">Frontline Forecast: live weather, radar, and forecast verification</h1>
      ...
      {workspaceNotice && <aside className="workspace-notice" role="status"><div><strong>{workspaceNotice.title ?? "Reference data added"}</strong><span>{workspaceNotice.message}</span></div><div>{workspaceNotice.targetDate && <button type="button" onClick={() => { const index = forecastRun.days.findIndex((day) => day.date === workspaceNotice.targetDate); if (index >= 0) setSelectedForecastDay(index); setActiveSection("forecast"); setWorkspaceNotice(null); }}>View forecast</button>}<button type="button" aria-label="Dismiss confirmation" onClick={() => setWorkspaceNotice(null)}>×</button></div></aside>}
      ...
      {homepageContent.showOutlook && <section className="outlook-strip" aria-label="Seven-day NWS guidance">
        ...
        <div className="outlook-cards" tabIndex={0} role="region" aria-label="Seven-day forecast, scrollable">{homeOutlook.length ? homeOutlook.map((day) => <article key={day.date}><strong>{day.label}</strong><b aria-hidden="true"><WeatherIcon description={day.shortForecast} style={weatherIconStyle} /></b><span>{day.shortForecast}</span><em>{day.high}° / {day.low}°</em><small>{day.precipitationChance ?? 0}% PoP</small></article>) : <p>Loading 7-day NWS guidance…</p>}</div>
      </section>}
      <section className="dashboard-grid">
        {homepageContent.showRadar && <article className="radar-card">
          ...
          <div className="radar">
            ...
            <RadarControlsMenu radarMapView={homeRadarMapView} onSelectView={selectHomeRadarView} radarProviderPreference={radarProviderPreference} onProviderPreferenceChange={setRadarProviderPreference} reflectivityLabel="Radar" showNwsAlerts={showNwsAlerts} onToggleAlerts={setShowNwsAlerts} showSevereMarkers={showSevereMarkers} onToggleSevereMarkers={setShowSevereMarkers} showStationPicker={showStationPicker} onToggleStationPicker={setShowStationPicker} radarOpacity={radarOpacity} onOpacityChange={setRadarOpacity} hasForecasterToolsAccess={hasForecasterToolsAccess} onLogin={() => setLoginMenuOpen(true)} caption={homeRadarMapView === "satellite" ? `Observed NOAA ${satelliteInfo.label} GeoColor imagery` : homeRadarMapView === "velocity" ? `Live base velocity · ${radarSourceLabels[radarSource ?? "provider"]} · ${selectedLocation.radarSite}` : `Live composite reflectivity · ${radarSourceLabels[radarSource ?? "provider"]} · ${selectedLocation.radarSite}`} />
            {homeRadarMapView === "satellite" ? <figure className="satellite-view" onClick={() => setSatellitePlaying(false)}><img src={satelliteFrame?.url ?? `https://cdn.star.nesdis.noaa.gov/${satelliteInfo.id}/ABI/CONUS/GEOCOLOR/1250x750.jpg?refresh=${radarRefreshToken}`} alt={`Latest NOAA ${satelliteInfo.label} GeoColor image for the continental United States${satelliteFrame ? ` at ${satelliteFrameTime}` : ""}`} /><figcaption>{satelliteInfo.label} GeoColor · {selectedLocation.name} is within this regional view</figcaption></figure> : <RadarMap location={selectedLocation} opacity={radarOpacity / 100} showReflectivity={homeRadarMapView === "composite" || homeRadarMapView === "velocity"} moment={homeRadarMapView === "velocity" ? "velocity" : "reflectivity"} showAlerts={showNwsAlerts} showSevereMarkers={showSevereMarkers} showStationPicker={showStationPicker} onStationSelect={selectRadarStation} refreshToken={radarRefreshToken} recenterToken={radarRecenterToken} timelineTileUrl={homeRadarMapView === "composite" ? radarFrame?.tileUrl : null} isCurrentFrame={isCurrentRadarFrame} inHouseFrameTime={homeRadarMapView === "composite" ? radarFrame?.inHouseTime ?? null : null} inHouseFrameSource={homeRadarMapView === "composite" && radarFrame?.source === "mosaic" ? "mosaic" : "nexrad"} forceProvider={radarProviderPreference === "iem"} radarViewMode={radarViewPreference} theme="dark" onSourceChange={setRadarSource} onFrameMeta={setRadarFrameMeta} onMapClick={() => setRadarPlaying(false)} prefetchTileUrls={homeRadarMapView === "composite" ? radarFrames.map((frame) => frame.tileUrl) : undefined} prefetchInHouseFrames={homeRadarMapView === "composite" ? radarFrames.filter((frame) => (frame.source === "nexrad" || frame.source === "mosaic") && frame.inHouseTime).map((frame) => ({ time: frame.inHouseTime as string, source: frame.source as "nexrad" | "mosaic" })) : undefined} />}
          </div>
          <RadarLegendStrip view={homeRadarMapView} inline elevationDeg={radarFrameMeta?.elevationDeg ?? null} observedAtLabel={radarObservedAtLabel} satelliteLabel={satelliteInfo.label} />
          {homeRadarMapView === "composite" && <div className="radar-playback"><button type="button" disabled={radarFrames.length < 2} onClick={() => setRadarPlaying((playing) => !playing)}>{radarPlaying ? "Pause" : "Play"}</button><input type="range" aria-label="Radar time" className="radar-scrub" min="0" max={Math.max(0, radarFrames.length - 1)} value={radarFrameIndex} disabled={radarFrames.length < 2} onChange={(event) => { setRadarPlaying(false); setRadarFrameIndex(Number(event.target.value)); }} /><span>{radarFrames.length ? radarFrameTime : radarTimelineStatus}</span></div>}
          {homeRadarMapView === "satellite" && satelliteFrames.length > 0 && <div className="radar-playback"><button type="button" disabled={satelliteFrames.length < 2} onClick={() => setSatellitePlaying((playing) => !playing)}>{satellitePlaying ? "Pause" : "Play"}</button><input type="range" aria-label="Radar time" className="radar-scrub" min="0" max={Math.max(0, satelliteFrames.length - 1)} value={satelliteFrameIndex} disabled={satelliteFrames.length < 2} onChange={(event) => { setSatellitePlaying(false); setSatelliteFrameIndex(Number(event.target.value)); }} /><span>{satelliteFrameTime}</span></div>}
        </article>}
      ...
      {liveWeather && liveWeather.hourly.length > 0 && <section className="outlook-strip hourly-strip" aria-label="Next 12 hours">
        ...
        <div className="hourly-cards" tabIndex={0} role="region" aria-label="Next twelve hours, scrollable">{liveWeather.hourly.map((hour) => <article key={hour.startTime}>
      ...
      {homepageContent.showReferences && <section className="data-desk">
        ...
        <div className="tabs" role="tablist" aria-label="Forecast data categories">
          {DATA_DESK_CATEGORIES.map((category) => <button key={category.id} type="button" role="tab" aria-selected={DATA_PANEL_CATEGORY[dataPanel] === category.id} className={DATA_PANEL_CATEGORY[dataPanel] === category.id ? "active" : ""} onClick={() => setDataPanel(category.panels[0].id)}>{category.label}</button>)}
        </div>
        <div className="data-desk-subtabs" role="tablist" aria-label={`${DATA_DESK_CATEGORIES.find((category) => category.id === DATA_PANEL_CATEGORY[dataPanel])?.label ?? ""} views`}>
          {DATA_DESK_CATEGORIES.find((category) => category.id === DATA_PANEL_CATEGORY[dataPanel])?.panels.map((panel) => <button key={panel.id} type="button" role="tab" aria-selected={dataPanel === panel.id} className={dataPanel === panel.id ? "active" : ""} onClick={() => setDataPanel(panel.id)}>{panel.label}</button>)}
        </div>
        ...
        {dataPanel === "sounding" && !hasForecasterToolsAccess && <SignInToolsUpsell label="Sounding" onLogin={() => setLoginMenuOpen(true)} />}
        {dataPanel === "sounding" && hasForecasterToolsAccess && <section className="observed-sounding-panel"><div className="model-guidance-heading"><div><strong>Latest observed K{selectedLocation.upperAirStation} sounding</strong><span>Nearest upper-air site for {selectedLocation.name} · official SPC analysis panel</span></div><a href={`https://www.spc.noaa.gov/exper/soundings/LATEST/${selectedLocation.upperAirStation}.gif`} target="_blank" rel="noreferrer">Open SPC source</a></div><img src={officialSoundingImageUrl(selectedLocation.upperAirStation)} alt={`Latest observed K${selectedLocation.upperAirStation} upper-air sounding from the Storm Prediction Center`} /><details><summary>Raw K{selectedLocation.upperAirStation} sounding text</summary><pre className="model-text">{soundingText || soundingStatus}</pre></details></section>}
        {dataPanel === "models" && !hasForecasterToolsAccess && <SignInToolsUpsell label="Model data" onLogin={() => setLoginMenuOpen(true)} />}
        {dataPanel === "models" && hasForecasterToolsAccess && <section className="model-workspace">
        ...
        {dataPanel === "model-sounding" && hasForecasterToolsAccess && <section className="model-sounding-panel">
        ...
        {dataPanel !== "models" && session && <div className="desk-reference-action"><div><strong>Add to forecast</strong><small>Save this view with the matching forecast date.</small></div><button type="button" onClick={pinCurrentDeskPanel}>Add reference</button></div>}
      </section>}
      </>}
      ...
      {activeSection === "radar" && <section className="radar-workspace">
        ...
        <RadarProductStrip radarMapView={radarMapView} onSelectView={selectRadarView} radarWorkspaceTab={radarWorkspaceTab} onSelectWorkspaceTab={setRadarWorkspaceTab} />
        {radarWorkspaceTab === "fronts" ? <div className="radar radar-workspace-map fronts-map-wrapper">{hasForecasterToolsAccess ? <FrontsMap /> : <SignInToolsUpsell label="Surface fronts" onLogin={() => setLoginMenuOpen(true)} />}</div> : radarWorkspaceTab === "upper-air" ? <div className="radar radar-workspace-map">{hasForecasterToolsAccess ? <UpperAirMap /> : <SignInToolsUpsell label="Upper-air observations" onLogin={() => setLoginMenuOpen(true)} />}</div> : <>
        {radarMapView === "future_reflectivity" && !hasForecasterToolsAccess ? <div className="radar radar-workspace-map"><SignInToolsUpsell label="Model radar" onLogin={() => setLoginMenuOpen(true)} /></div> : <div className="radar radar-workspace-map">
          <button type="button" className="radar-recenter-btn" title="Recenter" aria-label="Recenter radar" onClick={() => setRadarRecenterToken((value) => value + 1)}>⌖</button>
          <RadarControlsMenu radarMapView={radarMapView} onSelectView={selectRadarView} radarProviderPreference={radarProviderPreference} onProviderPreferenceChange={setRadarProviderPreference} reflectivityLabel="Reflectivity" showNwsAlerts={showNwsAlerts} onToggleAlerts={setShowNwsAlerts} showOutlookToggle showSpcOutlook={showSpcOutlook} onToggleOutlook={setShowSpcOutlook} outlookDay={outlookDay} onOutlookDayChange={setOutlookDay} showSevereMarkers={showSevereMarkers} onToggleSevereMarkers={setShowSevereMarkers} showStationPicker={showStationPicker} onToggleStationPicker={setShowStationPicker} radarOpacity={radarOpacity} onOpacityChange={setRadarOpacity} hasForecasterToolsAccess={hasForecasterToolsAccess} onLogin={() => setLoginMenuOpen(true)} caption={radarMapView === "satellite" ? `NOAA ${satelliteInfo.label} GeoColor imagery.` : radarMapView === "future_reflectivity" ? "HRRR simulated reflectivity — model guidance, not an observation." : radarMapView === "velocity" ? `Live base velocity · ${radarSourceLabels[radarSource ?? "provider"]} · ${selectedLocation.radarSite}.` : `Live composite reflectivity · ${radarSourceLabels[radarSource ?? "provider"]} · ${selectedLocation.radarSite}.`} />
          {radarMapView === "satellite" ? <figure className="satellite-view" onClick={() => setSatellitePlaying(false)}><div className="radar-field-picker satellite-channel-picker">{(["geocolor", "ir", "wv"] as const).map((channel) => <button type="button" key={channel} className={satelliteChannel === channel ? "active" : ""} onClick={(event) => { event.stopPropagation(); setSatelliteChannel(channel); setSatellitePlaying(false); }}>{({ geocolor: "GeoColor", ir: "Infrared", wv: "Water vapor" } as Record<string, string>)[channel]}</button>)}</div><img src={satelliteFrame?.url ?? `https://cdn.star.nesdis.noaa.gov/${satelliteInfo.id}/ABI/CONUS/${{ geocolor: "GEOCOLOR", ir: "13", wv: "08" }[satelliteChannel]}/1250x750.jpg?refresh=${radarRefreshToken}`} alt={`NOAA ${satelliteInfo.label} ${{ geocolor: "GeoColor", ir: "clean infrared", wv: "upper-level water vapor" }[satelliteChannel]} image for the continental United States${satelliteFrame ? ` at ${satelliteFrameTime}` : ""}`} /><figcaption>{({ geocolor: `Observed NOAA ${satelliteInfo.label} GeoColor`, ir: `Observed NOAA ${satelliteInfo.label} clean infrared (cloud-top temperature)`, wv: `Observed NOAA ${satelliteInfo.label} upper-level water vapor` } as Record<string, string>)[satelliteChannel]} · {selectedLocation.name}</figcaption></figure> : radarMapView === "future_reflectivity" ? <RadarMap location={selectedLocation} opacity={radarOpacity / 100} showReflectivity showAlerts={false} showStationPicker={showStationPicker} onStationSelect={selectRadarStation} recenterToken={radarRecenterToken} timelineTileUrl={futureRadarFrame?.tileUrl ?? null} isCurrentFrame={false} theme="dark" onMapClick={() => setFutureRadarPlaying(false)} /> : <RadarMap location={selectedLocation} opacity={radarOpacity / 100} showReflectivity={radarMapView === "composite" || radarMapView === "velocity"} moment={radarMapView === "velocity" ? "velocity" : "reflectivity"} showAlerts={showNwsAlerts} showOutlook={showSpcOutlook} outlookDay={outlookDay} showSevereMarkers={showSevereMarkers} showStationPicker={showStationPicker} onStationSelect={selectRadarStation} refreshToken={radarRefreshToken} recenterToken={radarRecenterToken} timelineTileUrl={radarMapView === "composite" ? radarFrame?.tileUrl : null} isCurrentFrame={isCurrentRadarFrame} inHouseFrameTime={radarMapView === "composite" ? radarFrame?.inHouseTime ?? null : null} inHouseFrameSource={radarMapView === "composite" && radarFrame?.source === "mosaic" ? "mosaic" : "nexrad"} forceProvider={radarProviderPreference === "iem"} radarViewMode={radarViewPreference} theme="dark" onSourceChange={setRadarSource} onFrameMeta={setRadarFrameMeta} onMapClick={() => setRadarPlaying(false)} prefetchTileUrls={radarMapView === "composite" ? radarFrames.map((frame) => frame.tileUrl) : undefined} prefetchInHouseFrames={radarMapView === "composite" ? radarFrames.filter((frame) => (frame.source === "nexrad" || frame.source === "mosaic") && frame.inHouseTime).map((frame) => ({ time: frame.inHouseTime as string, source: frame.source as "nexrad" | "mosaic" })) : undefined} />}
        </div>}
        {radarMapView === "future_reflectivity" && hasForecasterToolsAccess && <div className="model-forecast-banner">Model guidance — not an observation. HRRR simulated reflectivity, styled like radar; treat it as less certain the further out it goes.</div>}
        {(radarMapView !== "future_reflectivity" || hasForecasterToolsAccess) && <RadarLegendStrip view={radarMapView} inline elevationDeg={radarFrameMeta?.elevationDeg ?? null} observedAtLabel={radarObservedAtLabel} satelliteLabel={satelliteInfo.label} />}
        {radarMapView === "composite" && <div className="radar-playback"><button type="button" disabled={radarFrames.length < 2} onClick={() => setRadarPlaying((playing) => !playing)}>{radarPlaying ? "Pause" : "Play"}</button><input type="range" aria-label="Radar time" className="radar-scrub" min="0" max={Math.max(0, radarFrames.length - 1)} value={radarFrameIndex} disabled={radarFrames.length < 2} onChange={(event) => { setRadarPlaying(false); setRadarFrameIndex(Number(event.target.value)); }} /><span>{radarFrames.length ? radarFrameTime : radarTimelineStatus}</span></div>}
        {radarMapView === "satellite" && satelliteFrames.length > 0 && <div className="radar-playback"><button type="button" disabled={satelliteFrames.length < 2} onClick={() => setSatellitePlaying((playing) => !playing)}>{satellitePlaying ? "Pause" : "Play"}</button><input type="range" aria-label="Radar time" className="radar-scrub" min="0" max={Math.max(0, satelliteFrames.length - 1)} value={satelliteFrameIndex} disabled={satelliteFrames.length < 2} onChange={(event) => { setSatellitePlaying(false); setSatelliteFrameIndex(Number(event.target.value)); }} /><span>{satelliteFrameTime}</span></div>}
        {radarMapView === "future_reflectivity" && futureRadarFrames.length > 0 && <div className="radar-playback"><button type="button" disabled={futureRadarFrames.length < 2} onClick={() => setFutureRadarPlaying((playing) => !playing)}>{futureRadarPlaying ? "Pause" : "Play"}</button><input type="range" aria-label="Radar time" className="radar-scrub" min="0" max={Math.max(0, futureRadarFrames.length - 1)} value={futureRadarFrameIndex} disabled={futureRadarFrames.length < 2} onChange={(event) => { setFutureRadarPlaying(false); setFutureRadarFrameIndex(Number(event.target.value)); }} /><span>{futureRadarFrameTime}</span></div>}
        </>}
      </section>}
      ...
      {activeSection === "forecast" && session && <section className="workspace-card">
        ...
        <form id="forecast-form" onSubmit={handleForecastFormSubmit} onKeyDown={advanceForecastEntry}><div className="forecast-period-columns">
          <fieldset className="forecast-period"><legend>{new Intl.DateTimeFormat("en-US", { weekday: "long", month: "long", day: "numeric" }).format(new Date(`${selectedDay.date}T12:00:00`))} day <small>7 AM–7 PM</small></legend><div className="forecast-fields">
            ...
            <label>Dew point<span className="unit-input" style={unitInputStyle(temperatureInputValue(selectedDay.day.dewpoint), 2)}><input inputMode="decimal" placeholder="58" value={temperatureInputValue(selectedDay.day.dewpoint)} onChange={(event) => updatePeriod("day", "dewpoint", temperatureInputValue(event.target.value))} onBlur={() => formatPeriodField("day", "dewpoint")} /><i aria-hidden="true">°</i></span></label>
          ...
          <fieldset className="forecast-period"><legend>{new Intl.DateTimeFormat("en-US", { weekday: "long", month: "long", day: "numeric" }).format(new Date(`${selectedDay.date}T12:00:00`))} night <small>7 PM–7 AM</small></legend><div className="forecast-fields">
            ...
            <label>Dew point<span className="unit-input" style={unitInputStyle(temperatureInputValue(selectedDay.night.dewpoint), 2)}><input inputMode="decimal" placeholder="55" value={temperatureInputValue(selectedDay.night.dewpoint)} onChange={(event) => updatePeriod("night", "dewpoint", temperatureInputValue(event.target.value))} onBlur={() => formatPeriodField("night", "dewpoint")} /><i aria-hidden="true">°</i></span></label>
      ...
      {activeSection === "verify" && session && verifyTab === "records" && <section className="workspace-card">
        ...
        <section className="date-record-list"><div><strong>{forecastTargetTitle(recordFocusDate)}</strong><small>{focusedDateRecords.length ? `${focusedDateRecords.length} forecast${focusedDateRecords.length === 1 ? "" : "s"} for this date · right-click one for actions` : "No saved forecasts for this date"}</small></div>{focusedDateRecords.length ? <div>{focusedDateRecords.map((archive) => { const verification = automaticVerifications[archive.id]; return <button type="button" key={archive.id} className={archive.id === selectedArchiveId ? "active" : ""} onClick={() => setSelectedArchiveId(archive.id)} onContextMenu={(event) => { event.preventDefault(); setArchiveMenuId(archive.id); setArchiveMenuPosition({ left: event.clientX, top: event.clientY }); }}><strong>V{archive.versionNumber} · {archive.status}</strong><span>H {displayForecastTemperature(archive.day.high)} · L {displayForecastTemperature(archive.night.low)} · PoP {displayForecastChance(archive.day.rainChance)}/{displayForecastChance(archive.night.rainChance)}</span><small>{forecastAuthorLabel(archive, profiles, session.user.id)} · {verification?.dayScore !== null && verification?.dayScore !== undefined ? `Day score ${verification.dayScore}%` : "Unscored"}</small></button>; })}</div> : <p className="empty">No saved forecasts for this date.</p>}</section>
      ...
      {activeSection === "control" && session && isFirstRunSetup && <section className="workspace-card control-panel control-setup-card"><header><p className="eyebrow">Welcome</p><h2>Finish creating your account</h2><p>You're signing in as {session.user.email}. Add your name and set a password to get started.</p></header><form onSubmit={(event) => { event.preventDefault(); completeAccountSetup(); }} className="settings-grid"><label>Your name<input value={setupDisplayName} onChange={(event) => setSetupDisplayName(event.target.value)} placeholder="How instructors and classmates will see you" autoFocus /></label><label>Email<input type="email" value={setupEmail} onChange={(event) => setSetupEmail(event.target.value)} placeholder="you@example.com" /></label><label>Password<input type="password" autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder="At least 8 characters" /></label><label>Confirm password<input type="password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="Repeat password" /></label><div className="settings-actions"><button type="submit" disabled={passwordBusy || !newPassword || !confirmPassword}>{passwordBusy ? "Setting up…" : "Finish creating my account"}</button></div></form>{passwordMessage && <p className="control-message" role="status">{passwordMessage}</p>}</section>}
      {activeSection === "control" && session && !isFirstRunSetup && <div className="section-heading"><div><p className="eyebrow">Account</p><h2>{workspaceNavigation.find((item) => item.id === "control")?.label || "Settings"}</h2><p>Your profile, plan, and how Frontline Forecast looks and opens for you.</p></div></div>}
      {activeSection === "control" && session && !isFirstRunSetup && <nav className="classroom-hub-tabs" aria-label="Settings sections">
        <button type="button" className={controlTab === "profile" ? "active" : ""} onClick={() => setControlTab("profile")}>Profile</button>
        <button type="button" className={controlTab === "plan" ? "active" : ""} onClick={() => setControlTab("plan")}>Plan</button>
        <button type="button" className={controlTab === "preferences" ? "active" : ""} onClick={() => setControlTab("preferences")}>Preferences</button>
        <button type="button" className={controlTab === "security" ? "active" : ""} onClick={() => setControlTab("security")}>Security</button>
        <button type="button" className={controlTab === "data" ? "active" : ""} onClick={() => setControlTab("data")}>Data</button>
      </nav>}
      {activeSection === "control" && session && !isFirstRunSetup && controlTab === "profile" && <section className="workspace-card control-panel control-profile-card"><header><h3>Profile</h3><p>Your name and sign-in email.</p></header><form onSubmit={(event) => { event.preventDefault(); saveProfile(); }} className="settings-grid"><label>Your name<input value={profileDraftName} onChange={(event) => setProfileDraftName(event.target.value)} placeholder="How instructors and classmates will see you" /></label><label>Email<input type="email" value={profileDraftEmail} onChange={(event) => setProfileDraftEmail(event.target.value)} /><small>Changing this sends a confirmation link to the new address.</small></label><div className="settings-actions"><button type="submit" disabled={myProfileBusy}>{myProfileBusy ? "Saving…" : "Save profile"}</button></div></form>{myProfileMessage && <p className="control-message" role="status">{myProfileMessage}</p>}</section>}
      {activeSection === "control" && session && !isFirstRunSetup && controlTab === "plan" && <section className="workspace-card control-panel plan-card"><header><h3>Your plan</h3><p>Model data, ensembles, and simulated reflectivity are part of Personal+. Live observations, radar, and the forecast workspace are on every plan.</p></header><div className="plan-status"><b className={hasSchoolMembership || personalTier === "paid" ? "status-ready" : "status-pending"}>{hasSchoolMembership ? "Included via school" : personalTier === "paid" ? "Personal+" : "Free"}</b>{hasSchoolMembership ? <span>Your school or class workspace already includes Personal+ features — no request needed.</span> : personalTier === "paid" ? <span>You have Personal+ access on this account.</span> : <span>Pricing is still being finalized while Frontline Forecast is pre-launch. Request access below and we will follow up by email.</span>}</div>{!hasSchoolMembership && personalTier !== "paid" && (pendingTierRequest ? <p className="control-message" role="status">Request pending review since {new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(new Date(pendingTierRequest.created_at))}. Approved yet? <button type="button" className="login-menu-link" onClick={() => setProfileRefreshToken((value) => value + 1)}>Check again</button></p> : <form onSubmit={(event) => { event.preventDefault(); submitTierRequest(); }} className="settings-grid"><label>Note (optional)<textarea value={tierNote} onChange={(event) => setTierNote(event.target.value)} placeholder="Anything we should know?" rows={2} /></label><div className="settings-actions"><button type="submit" disabled={tierRequestBusy}>{tierRequestBusy ? "Sending…" : "Request Personal+ access"}</button></div></form>)}<div className="settings-actions"><button type="button" onClick={() => setJoinPanelOpen(true)}>Have a school or class code?</button></div>{tierRequestMessage && <p className="control-message" role="status">{tierRequestMessage}</p>}</section>}
      {activeSection === "control" && session && !isFirstRunSetup && controlTab === "plan" && hasControlAccess && <section className="workspace-card control-panel"><header><h3>Tier requests</h3><p>Approve or deny Personal+ access until billing is wired up.</p></header><div className="access-roster-list">{adminTierRequests.map((request) => <div key={request.id}><span><strong>{request.profiles?.display_name || request.profiles?.email || request.user_id}</strong><small>{request.note || "No note"} · {new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(new Date(request.created_at))}</small></span><button type="button" onClick={() => resolveTierRequest(request.id, true)}>Approve</button><button type="button" onClick={() => resolveTierRequest(request.id, false)}>Deny</button></div>)}</div>{!adminTierRequests.length && <p className="empty">No pending tier requests.</p>}{adminTierMessage && <p className="control-message" role="status">{adminTierMessage}</p>}</section>}
      {activeSection === "control" && session && !isFirstRunSetup && controlTab === "preferences" && <section className="workspace-card control-panel control-defaults-card"><header><h3>Your defaults</h3><p>Location, forecast length, and appearance stay on this browser. Weather symbols and radar view follow your account to any device.</p></header><div className="settings-grid"><label>Default location<select value={defaultLocationId} onChange={(event) => setDefaultLocationId(weatherDeskLocation(event.target.value).id)}>{weatherDeskLocations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select><small>This browser only.</small></label><label>New forecast horizon<select value={defaultForecastDays} onChange={(event) => setDefaultForecastDays(Number(event.target.value) as 1 | 3 | 7)}><option value={1}>1 day</option><option value={3}>3 days</option><option value={7}>7 days</option></select><small>This browser only.</small></label><label>Dark mode<select value={theme} onChange={(event) => setTheme(event.target.value as "light" | "dark")}><option value="light">Light mode</option><option value="dark">Dark mode</option></select><small>This browser only.</small></label><label>Accent color<select value={accentPreference} onChange={(event) => setAccentPreference(event.target.value as AccentPreference)}><option value="default">Sky blue (default)</option><option value="deep-navy">Deep navy</option><option value="storm-red">Storm red</option></select><small>This browser only.</small></label><label>Card style<select value={cardStyle} onChange={(event) => setCardStyle(event.target.value as CardStyle)}><option value="flat">Flat</option><option value="raised">Raised</option></select><small>This browser only.</small></label><label>Weather symbols<select value={weatherIconStyle} onChange={(event) => saveWeatherIconStyle(event.target.value as WeatherIconStyle)}><option value="traditional">Traditional</option><option value="minimal">Minimal</option></select><small>Saved to your account.</small></label><label>Radar view<select value={radarViewPreference} onChange={(event) => saveRadarViewPreference(event.target.value as "mosaic" | "station")}><option value="mosaic">Local mosaic</option><option value="station">Single station</option></select><small>Saved to your account. Local mosaic combines your nearest stations into one wider view.</small></label></div><div className="settings-actions"><button type="button" onClick={() => setControlMessage("Your defaults are saved.")}>Save</button><button type="button" onClick={() => { setDefaultLocationId(defaultWeatherDeskLocation.id); setDefaultForecastDays(1); setTheme("light"); setAccentPreference("default"); setCardStyle("flat"); saveWeatherIconStyle("traditional"); saveRadarViewPreference("mosaic"); setControlMessage("Defaults restored."); }}>Restore defaults</button></div>{controlMessage && <p className="control-message" role="status">{controlMessage}</p>}</section>}
      {activeSection === "control" && session && !isFirstRunSetup && controlTab === "security" && <section className="workspace-card control-panel control-password-card"><header><h3>Change password</h3><p>Update the password used to sign in to Frontline Forecast.</p></header><form onSubmit={(event) => { event.preventDefault(); changePassword(); }} className="settings-grid"><label>New password<input type="password" autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder="At least 8 characters" /></label><label>Confirm new password<input type="password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="Repeat new password" /></label><div className="settings-actions"><button type="submit" disabled={passwordBusy || !newPassword || !confirmPassword}>{passwordBusy ? "Updating…" : "Update password"}</button></div></form>{passwordMessage && <p className="control-message" role="status">{passwordMessage}</p>}</section>}
      {activeSection === "control" && session && !isFirstRunSetup && controlTab === "data" && <section className="workspace-card control-panel control-data-card"><header><h3>Your data</h3><p>Download a copy of your account, forecasts, and reviews.</p></header><div className="settings-actions"><button type="button" onClick={exportMyData}>Export my data</button></div>{exportMessage && <p className="control-message" role="status">{exportMessage}</p>}</section>}
      {activeSection === "control" && session && !isFirstRunSetup && controlTab === "data" && <section className="workspace-card control-panel control-delete-card"><header><h3>Delete account</h3><p>Permanently removes your login and profile information. See our <a href="/privacy">Privacy Policy</a> for what happens to coursework tied to a school.</p></header><form onSubmit={(event) => { event.preventDefault(); deleteAccount(); }} className="settings-grid"><label>Type DELETE to confirm<input value={deleteConfirmText} onChange={(event) => setDeleteConfirmText(event.target.value)} placeholder="DELETE" /></label><div className="settings-actions"><button type="submit" className="danger" disabled={deleteBusy || deleteConfirmText.trim().toUpperCase() !== "DELETE"}>{deleteBusy ? "Deleting…" : "Delete my account"}</button></div></form>{deleteMessage && <p className="control-message" role="status">{deleteMessage}</p>}</section>}
      ...
      {activeSection === "verify" && session && verifyTab === "records" && selectedArchive && <section className="workspace-card record-actions-card"><div className="record-actions"><div><strong>Archive actions</strong><small>Revisions create a new auditable forecast. Removing a submitted forecast withdraws it from your working archive while retaining its protected history.</small></div><div><button type="button" onClick={() => reviseArchive(selectedArchive)}>Revise forecast</button><button type="button" className="danger" onClick={() => requestArchiveRemoval(selectedArchive)}>{selectedArchive.status === "draft" ? "Delete draft" : "Withdraw forecast"}</button></div></div></section>}
    </main>
