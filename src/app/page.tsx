...
export default function Home() {
  ...
  const [schoolEntitlement, setSchoolEntitlement] = useState<{ seat_limit: number; class_limit: number; class_seat_limit: number; status: string; next_payment_due_at: string | null } | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`${supabaseUrl}/rest/v1/organization_entitlements?select=seat_limit,class_limit,class_seat_limit,status,next_payment_due_at&organization_id=eq.${organizationId}`, { headers }),
      ...
      const entitlementRows = entitlementResponse.ok ? await entitlementResponse.json() as { seat_limit: number; class_limit: number; class_seat_limit: number; status: string; next_payment_due_at: string | null }[] : [];
