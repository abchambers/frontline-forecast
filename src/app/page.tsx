  ...
  return (
    ...
    <main className={`${radarExpanded ? "app radar-expanded" : "app"} desk-${activeWorkspace?.kind ?? "public"}`}>
      ...
      {activeSection === "radar" && <section className="radar-workspace">
        ...
        {radarMapView === "composite" && <div className="radar-playback"><button type="button" disabled={radarFrames.length < 2} onClick={() => setRadarPlaying((playing) => !playing)}>{radarPlaying ? "Pause" : "Play"}</button><input type="range" className="radar-scrub" min="0" max={Math.max(0, radarFrames.length - 1)} value={radarFrameIndex} disabled={radarFrames.length < 2} onChange={(event) => { setRadarPlaying(false); setRadarFrameIndex(Number(event.target.value)); }} /><span>{radarFrames.length ? radarFrameTime : radarTimelineStatus}</span></div>}
        <div className="radar-workspace-footer"><RadarLegendStrip view={radarMapView} inline elevationDeg={radarFrameMeta?.elevationDeg ?? null} observedAtLabel={radarObservedAtLabel} /><p>Observed products show what has happened or is happening. Live and past frames come from the same NWS/NEXRAD mosaic (via the Iowa Environmental Mesonet), so they always match. Model-derived guidance styled like radar lives under Models &amp; Observations, not here.</p></div>
      </section>}
