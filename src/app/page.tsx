  ...
  return (
    ...
    <main className={`app desk-${activeWorkspace?.kind ?? "public"}`}>
        ...
        <div className="header-meta">
          ...
          {session ? <div className="avatar-menu-wrap">
            ...
            {workspaceMenuOpen && <div className="avatar-menu">
              ...
              {soleStudentDeskKey && activeWorkspaceKey === soleStudentDeskKey ? <div className="avatar-menu-desk"><span className="avatar-menu-label">Desk</span><strong>{workspaceDeskLabel(activeWorkspace)}</strong><button type="button" className="workspace-join-link" onClick={() => { setJoinPanelOpen(true); setWorkspaceMenuOpen(false); }}>Join another class</button></div> : <div className="avatar-menu-desk"><button type="button" className="avatar-menu-desk-trigger" aria-expanded={deskListOpen} onClick={() => setDeskListOpen((open) => !open)}><span>Your desks</span><i aria-hidden="true">⌄</i></button>{deskListOpen && <div className="avatar-menu-desk-list">{workspaceContexts.map((workspace) => <button type="button" key={workspace.key} className={workspace.key === activeWorkspaceKey ? "active" : ""} onClick={() => { switchWorkspace(workspace); setDeskListOpen(false); }}><strong>{workspaceDeskLabel(workspace)}</strong><span>{workspace.detail}{workspace.role ? ` · ${workspace.role}` : ""}</span></button>)}<button type="button" className="workspace-join-action" onClick={() => { setJoinPanelOpen(true); setWorkspaceMenuOpen(false); }}>Join a school or class</button>{workspaceContextStatus && <em>{workspaceContextStatus}</em>}</div>}</div>}
              <div className="avatar-menu-actions">{activeWorkspace?.kind === "organization" && <button type="button" onClick={() => { setActiveSection("school"); setWorkspaceMenuOpen(false); }}>Open school</button>}{activeWorkspace?.kind === "classroom" && <button type="button" onClick={() => { setActiveSection("classroom"); setWorkspaceMenuOpen(false); }}>Open class</button>}{visibleWorkspace("control") && <button type="button" onClick={() => { setActiveSection("control"); setWorkspaceMenuOpen(false); }}>{workspaceNavigation.find((item) => item.id === "control")?.label || "Settings"}</button>}</div>
      ...
      {activeSection === "radar" && <section className="radar-workspace">
        ...
        <RadarLegendStrip view={radarMapView} inline elevationDeg={radarFrameMeta?.elevationDeg ?? null} observedAtLabel={radarObservedAtLabel} />
        {radarMapView === "composite" && <div className="radar-playback"><button type="button" disabled={radarFrames.length < 2} onClick={() => setRadarPlaying((playing) => !playing)}>{radarPlaying ? "Pause" : "Play"}</button><input type="range" className="radar-scrub" min="0" max={Math.max(0, radarFrames.length - 1)} value={radarFrameIndex} disabled={radarFrames.length < 2} onChange={(event) => { setRadarPlaying(false); setRadarFrameIndex(Number(event.target.value)); }} /><span>{radarFrames.length ? radarFrameTime : radarTimelineStatus}</span></div>}
        <div className="radar-workspace-footer"><p>Observed products show what has happened or is happening. Live and past frames come from the same NWS/NEXRAD mosaic (via the Iowa Environmental Mesonet), so they always match. Model-derived guidance styled like radar lives under Models &amp; Observations, not here.</p></div>
      </section>}
