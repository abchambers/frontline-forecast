...
type RadarProductMode = "observed" | "forecast";
...
const defaultWorkspaceNavigation: WorkspaceNavigationItem[] = [
  ...
  { id: "control", label: "Control panel", access: "owner", enabled: true },
];
...
export default function Home() {
  ...
  const [schoolEntitlement, setSchoolEntitlement] = useState<{ class_limit: number; class_seat_limit: number; status: string } | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (settings.radarMapView && ["composite", "satellite", "base", "precipitation_new", "clouds_new", "pressure_new", "wind_new", "temp_new", "ndfd_maxt", "ndfd_pop12", "ndfd_windspd"].includes(settings.radarMapView)) {
        setRadarMapView(settings.radarMapView);
        setRadarProductMode(["ndfd_maxt", "ndfd_pop12", "ndfd_windspd"].includes(settings.radarMapView) ? "forecast" : "observed");
      }
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`${supabaseUrl}/rest/v1/organization_entitlements?select=class_limit,class_seat_limit,status&organization_id=eq.${organizationId}`, { headers }),
      ...
      const entitlementRows = entitlementResponse.ok ? await entitlementResponse.json() as { class_limit: number; class_seat_limit: number; status: string }[] : [];
  ...
  function selectRadarView(view: RadarMapView) {
    ...
    setRadarProductMode(["ndfd_maxt", "ndfd_pop12", "ndfd_windspd"].includes(view) ? "forecast" : "observed");
  ...
  return (
    <main className={`${radarExpanded ? "app radar-expanded" : "app"} desk-${activeWorkspace?.kind ?? "public"}`}>
      ...
      <nav aria-label="Main navigation" className="navigation">
        ...
        {visibleWorkspace("control") && <button className={activeSection === "control" ? "active" : ""} onClick={() => setActiveSection("control")}>{workspaceNavigation.find((item) => item.id === "control")?.label || "Control panel"}</button>}
      </nav>
      ...
      {activeSection === "radar" && <section className="radar-workspace">
        ...
        <div className="radar radar-workspace-map">{radarLoop && radarMapView === "composite" && <div className="radar-playback"><button type="button" aria-label="Previous radar frame" disabled={!radarFrames.length} onClick={() => { setRadarPlaying(false); setRadarFrameIndex((index) => Math.max(0, index - 1)); }}>‹</button><button type="button" disabled={radarFrames.length < 2} onClick={() => setRadarPlaying((playing) => !playing)}>{radarPlaying ? "Pause" : "Play"}</button><button type="button" aria-label="Next radar frame" disabled={!radarFrames.length} onClick={() => { setRadarPlaying(false); setRadarFrameIndex((index) => Math.min(radarFrames.length - 1, index + 1)); }}>›</button><span>{radarFrameTime}</span></div>}{radarMapView === "satellite" ? <figure className="satellite-view"><img src={`https://cdn.star.nesdis.noaa.gov/GOES19/ABI/CONUS/GEOCOLOR/1250x750.jpg?refresh=${radarRefreshToken}`} alt="Latest NOAA GOES-East GeoColor image for the continental United States" /><figcaption>Observed NOAA GOES-East GeoColor · {selectedLocation.name}</figcaption></figure> : <RadarMap location={selectedLocation} opacity={radarOpacity / 100} showReflectivity={radarMapView === "composite"} weatherLayer={radarProductMode === "forecast" ? radarMapView : "none"} showAlerts={showNwsAlerts} refreshToken={radarRefreshToken} recenterToken={radarRecenterToken} timelineTileUrl={radarLoop && radarMapView === "composite" ? radarFrame?.tileUrl : null} theme={theme} />}</div>
        <div className="radar-workspace-footer"><RadarLegendStrip view={radarMapView} /><p>{radarProductMode === "observed" ? "Observed products show what has happened or is happening. Timeline frames use the current RainViewer pilot source while NOAA/NWS remains the live composite fallback." : "Forecast guidance shows what a published forecast field anticipates. Simulated radar will be added only after model ingest and clear run/valid-time labeling are in place."}</p></div>
        <div className="radar-product-roadmap"><div><strong>Next radar capability</strong><small>Model precipitation and simulated reflectivity will use this same map, with a run time and valid time shown for every frame.</small></div><div><button type="button" disabled>Model precipitation <span>Planned</span></button><button type="button" disabled>Simulated reflectivity <span>Planned</span></button></div></div>
      </section>}
