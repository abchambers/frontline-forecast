...
type WorkspacePreferences = { defaultLocationId: string; radarMapView: RadarMapView; radarOpacity: number; showNwsAlerts: boolean; showSpcOutlook: boolean; showSevereMarkers: boolean; defaultForecastDays: 1 | 3 | 7 };
...
const defaultPublicNavigation: PublicNavigationItem[] = [
  ...
  { id: "about", label: "How it works", target: "about", access: "public", enabled: true },
  { id: "login", label: "Sign in", target: "login", access: "public", enabled: true },
];
const defaultWorkspaceNavigation: WorkspaceNavigationItem[] = [
  { id: "weather", label: "Weather", access: "public", enabled: true },
  { id: "radar", label: "Radar", access: "public", enabled: true },
  { id: "about", label: "How it works", access: "public", enabled: true },
...
export default function Home() {
  ...
  const visiblePublicNavigation = publicNavigation.filter((item) => item.enabled && (item.target !== "login" || !session) && (item.target !== "about" || !session) && (item.access === "public" || (item.access === "member" && Boolean(session)) || (item.access === "staff" && hasControlAccess) || (item.access === "owner" && role === "owner")));
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(workspaceSettingsStorageKey, JSON.stringify({ defaultLocationId, radarMapView, radarOpacity, showNwsAlerts, showSpcOutlook, showSevereMarkers, defaultForecastDays } satisfies WorkspacePreferences));
  }, [defaultForecastDays, defaultLocationId, radarMapView, radarOpacity, settingsReady, showNwsAlerts, showSpcOutlook, showSevereMarkers]);
    ...
    const loadRadarTimeline = () => fetch("/api/radar/frames", { cache: "no-store" })
      .then(async (response) => {
        ...
        if (isActive) {
          ...
          setRadarTimelineStatus(`${frames.length} frames · NWS/NEXRAD via IEM`);
        }
      })
  ...
  return (
    ...
    <main className={`${radarExpanded ? "app radar-expanded" : "app"} desk-${activeWorkspace?.kind ?? "public"}`}>
      <header className="header">
        <div className="brand-lockup brand-lockup-wordmark"><span className="theme-brand-lockup"><img className="brand-lockup-light" src="/brand/frontline-forecast-lockup-light.png" alt="Frontline Forecast" /><img className="brand-lockup-dark" src="/brand/frontline-forecast-lockup-dark.png" alt="Frontline Forecast" /></span><p className="eyebrow" data-hq-content="brand.tagline">Human-first forecasting workspace</p>{session && activeSchoolBranding && <div className="school-brand-lockup" aria-label={`${activeSchoolBranding.school_name || activeWorkspace.label} school workspace`}><span aria-hidden="true">×</span>{supabaseUrl && activeSchoolLogoPath && <img src={schoolLogoUrl(supabaseUrl, activeSchoolLogoPath)} alt={activeSchoolBranding.logo_alt || `${activeSchoolBranding.school_name || activeWorkspace.label} logo`} />}<strong>{activeSchoolBranding.school_name || activeWorkspace.label}</strong></div>}</div>
      ...
      <nav aria-label="Main navigation" className="navigation">
        ...
        {visibleWorkspace("control") && <button className={activeSection === "control" ? "active" : ""} onClick={() => setActiveSection("control")}>{workspaceNavigation.find((item) => item.id === "control")?.label || "Settings"}</button>}
      </nav>
      ...
      <section className="dashboard-grid">
        {homepageContent.showRadar && <article className="radar-card">
          <div className="card-heading"><div><h2>{radarMapView === "satellite" ? "Satellite" : homepageContent.radarTitle}</h2><p>{radarMapView === "satellite" ? `GOES-East GeoColor · current CONUS picture for ${selectedLocation.name}` : radarMapView === "velocity" ? `Base velocity · centered on ${selectedLocation.name}` : `${homepageContent.radarCaption} · centered on ${selectedLocation.name}`}</p></div><div className="actions"><button onClick={() => setRadarRecenterToken((value) => value + 1)}>Recenter</button><button onClick={() => setRadarRefreshToken((value) => value + 1)}>Refresh</button><button onClick={() => setRadarExpanded((value) => !value)}>{radarExpanded ? "Exit expanded view" : "Expand radar"}</button></div></div>
          <div className="radar"><details className="radar-tools"><summary aria-label="Open radar controls">☰</summary><div><div className="radar-product-picker"><span>Data layer</span><div><button type="button" className={radarMapView === "composite" ? "active" : ""} onClick={() => selectRadarView("composite")}>Radar</button><button type="button" className={radarMapView === "velocity" ? "active" : ""} onClick={() => selectRadarView("velocity")}>Velocity</button><button type="button" className={radarMapView === "satellite" ? "active" : ""} onClick={() => selectRadarView("satellite")}>Satellite</button></div></div>{radarMapView !== "satellite" && <><label className="alert-overlay-toggle"><input type="checkbox" checked={showNwsAlerts} onChange={(event) => { setShowNwsAlerts(event.target.checked); event.currentTarget.closest("details")?.removeAttribute("open"); }} /> NWS watches &amp; warnings</label><label className="alert-overlay-toggle"><input type="checkbox" checked={showSevereMarkers} onChange={(event) => { setShowSevereMarkers(event.target.checked); event.currentTarget.closest("details")?.removeAttribute("open"); }} /> Storm tracks &amp; severe markers</label><label className="alert-overlay-toggle"><input type="checkbox" checked={showStationPicker} onChange={(event) => { setShowStationPicker(event.target.checked); event.currentTarget.closest("details")?.removeAttribute("open"); }} /> Show radar stations</label><label>Opacity <input type="range" min="20" max="100" value={radarOpacity} onChange={(event) => setRadarOpacity(Number(event.target.value))} /> <span>{radarOpacity}%</span></label></>}<small>{radarMapView === "satellite" ? "Observed NOAA GOES-East GeoColor imagery" : radarMapView === "velocity" ? `Live base velocity · ${radarSourceLabels[radarSource ?? "provider"]} · ${selectedLocation.radarSite}` : `Live composite reflectivity · ${radarSourceLabels[radarSource ?? "provider"]} · ${selectedLocation.radarSite}`}</small></div></details>{radarMapView === "satellite" ? <figure className="satellite-view"><img src={`https://cdn.star.nesdis.noaa.gov/GOES19/ABI/CONUS/GEOCOLOR/1250x750.jpg?refresh=${radarRefreshToken}`} alt="Latest NOAA GOES-East GeoColor image for the continental United States" /><figcaption>GOES-East GeoColor · {selectedLocation.name} is within this regional view</figcaption></figure> : <RadarMap location={selectedLocation} opacity={radarOpacity / 100} showReflectivity={radarMapView === "composite" || radarMapView === "velocity"} moment={radarMapView === "velocity" ? "velocity" : "reflectivity"} showAlerts={showNwsAlerts} showSevereMarkers={showSevereMarkers} showStationPicker={showStationPicker} onStationSelect={selectRadarStation} refreshToken={radarRefreshToken} recenterToken={radarRecenterToken} timelineTileUrl={radarMapView === "composite" ? radarFrame?.tileUrl : null} isCurrentFrame={isCurrentRadarFrame} theme={theme} onSourceChange={setRadarSource} />}</div>
