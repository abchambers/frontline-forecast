...
export default function Home() {
  ...
  return (
    ...
    <main className={`${radarExpanded ? "app radar-expanded" : "app"} desk-${activeWorkspace?.kind ?? "public"}`}>
      ...
      <section className="home-intro"><div className="home-intro-copy"><div className="data-state-line" aria-live="polite"><span className={`sync-pill ${liveDataStatus.tone}`}><i aria-hidden="true" />{liveDataStatus.label}</span>{liveDataTimestamp && <small>Updated {liveDataTimestamp}</small>}</div></div><div className="home-intro-actions"><button type="button" onClick={() => setActiveSection("dashboard")}>{homepageContent.primaryAction}</button><button type="button" onClick={() => session ? setActiveSection("forecast") : setLoginMenuOpen(true)}>{session ? "Go to forecast" : homepageContent.secondaryAction}</button></div></section>
