  ...
  return (
    ...
    <main className={`${radarExpanded ? "app radar-expanded" : "app"} desk-${activeWorkspace?.kind ?? "public"}`}>
      ...
      {activeSection === "radar" && <section className="radar-workspace">
        ...
        <div className="radar-workspace-subtools"><strong>Observed product</strong><div className="radar-field-picker"><button type="button" className={radarMapView === "composite" ? "active" : ""} onClick={() => selectRadarView("composite")}>Reflectivity</button><button type="button" className={radarMapView === "satellite" ? "active" : ""} onClick={() => selectRadarView("satellite")}>Satellite</button></div><span>{radarMapView === "satellite" ? "NOAA GOES-East GeoColor imagery." : `Live composite reflectivity · NWS/NEXRAD (MRMS) · ${selectedLocation.radarSite}.`}</span></div>
