...
export default function Home() {
  ...
  function loadAccessManagement() {
    ...
    Promise.all([
      ...
      fetch(`${supabaseUrl}/rest/v1/organization_memberships?select=id,organization_id,user_id,role,status,profiles(id,email,display_name,person_type)&order=created_at.asc`, { headers }),
      fetch(`${supabaseUrl}/rest/v1/classroom_memberships?select=id,classroom_id,user_id,role,status,profiles(id,email,display_name,person_type)&order=created_at.asc`, { headers }),
      ...
      setOrganizationMembers(await organizationMemberResponse.json() as OrganizationMember[]);
      setClassroomMembers(await classroomMemberResponse.json() as ClassroomMember[]);
    }).catch((error: Error) => setAccessMessage(error.message));
  }

  useEffect(() => { loadAccessManagement(); }, [session, role]);
