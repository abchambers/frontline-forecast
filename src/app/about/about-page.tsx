...
export function AboutPage() {
  ...
  return <main className="about-app"><header className="about-header"><Link href="/" className="about-brand"><span className="theme-brand-mark about-header-mark" aria-hidden="true"><img className="brand-mark-light" src="/brand/frontline-forecast-logo-light.svg" alt="" /><img className="brand-mark-dark" src="/brand/frontline-forecast-logo-dark.svg" alt="" /></span><strong>Frontline Forecast</strong></Link><div><button type="button" className="theme-toggle" onClick={toggle}>{theme === "light" ? "Dark mode" : "Light mode"}</button><Link href="/">Forecast</Link></div></header><section className="about-hero"><span className="theme-brand-mark about-hero-mark" aria-hidden="true"><img className="brand-mark-light" src="/brand/frontline-forecast-logo-light.svg" alt="" /><img className="brand-mark-dark" src="/brand/frontline-forecast-logo-dark.svg" alt="" /></span><div><p className="eyebrow">{content.eyebrow}</p><h1>{content.title}</h1><p>{content.description}</p></div></section><section className="about-principles" aria-label="About Frontline Forecast">{content.principles.map((principle, index) => <article key={`${principle.title}-${index}`}><span>0{index + 1}</span><h2>{principle.title}</h2><p>{principle.body}</p></article>)}</section></main>;
}
