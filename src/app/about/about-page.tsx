...
export function AboutPage() {
  ...
  return <main className="about-app"><header className="about-header"><Link href="/" className="about-brand"><img src="/brand/frontline-forecast-big-icon.svg" alt="" /><strong>Frontline Forecast</strong></Link><div><button type="button" className="theme-toggle" onClick={toggle}>{theme === "light" ? "Dark mode" : "Light mode"}</button><Link href="/">Forecast</Link></div></header><section className="about-hero"><p className="eyebrow">{content.eyebrow}</p><h1>{content.title}</h1><p>{content.description}</p></section><section className="about-principles" aria-label="About Frontline Forecast">{content.principles.map((principle, index) => <article key={`${principle.title}-${index}`}><span>0{index + 1}</span><h2>{principle.title}</h2><p>{principle.body}</p></article>)}</section></main>;
}
